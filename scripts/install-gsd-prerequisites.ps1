...
function Install-ViaWinget {
    ...
    Write-Check $Name "install" "Installing via winget..." "Yellow"
    try {
        ...
        if ($LASTEXITCODE -eq 0 -or "$output" -match "Successfully installed") {
            Write-Check $Name "pass" "Installed successfully" "Green"
            return $true
        } else {
            Write-Check $Name "fail" "winget install failed. Try: winget install $PackageId" "Red"
            return $false
        }
    } catch {
...
function Install-ViaNpm {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Check $Name "fail" "npm install failed. Try: npm install -g $Package" "Red"
            return $false
        }
    } catch {
        Write-Check $Name "fail" "Install error: $($_.Exception.Message)" "Red"
    ...
    if ($exists -and -not $Force) {
        ...
        if ($ver) {
            ...
            if ($versionOk) {
                ...
                Write-Check $tool.Name "warn" "v$($ver.Full) - minimum v$($tool.MinMajor).x required" "Yellow"
                $results.Warnings++
            }
        } else {
    ...
    if (-not $exists -or $Force) {
        if ($tool.NpmPackage) {
            if (-not (Test-CommandExists "npm")) {
                Write-Check $tool.Name "fail" "Cannot install - npm not available (install Node.js first)" "Red"
                $results.Failed++
                continue
            }
            ...
            Write-Check $tool.Name "fail" "Not installed. Install manually: winget install $($tool.WingetId)" "Red"
            $installed = $false
        } else {
            Write-Check $tool.Name "fail" "Not found. Comes with Node.js - install Node.js first" "Red"
            $installed = $false
        }
...
if ($results.Failed -eq 0 -and $results.Warnings -eq 0) {
    ...
    Write-Host ""
    if ($results.Installed -gt 0) {
        Write-Host "  Some tools were just installed. Restart terminal and re-run:" -ForegroundColor Yellow
