...
export class CodeReviewAgent extends BaseAgent {
  protected async run(input: AgentInput): Promise<AgentOutput> {
    ...
    const userMessage = [
      ...
      '## Build Results',
      '',
      `dotnet build: ${buildResult.dotnet.success ? 'PASS' : 'FAIL — ' + buildResult.dotnet.output.split('\n').filter(l => l.includes('error')).slice(0, 5).join('; ')}`,
      '',
      `npm build: ${buildResult.npm.success ? 'PASS' : 'FAIL — ' + buildResult.npm.output.split('\n').filter(l => l.includes('error') || l.includes('Error')).slice(0, 5).join('; ')}`,
      '',
      '## Test Results',
      '',
      ...
      '## Instructions',
      '',
      'Review the code changes against the convergence report in TWO passes:',
      '',
      '**Pass 1 — Standard Review:** Check correctness, security, style, coverage, convergence.',
      '**Pass 2 — Adversarial Review:** Challenge design decisions in the changed code:',
      '- Is this the simplest approach? Could it be done with fewer files/abstractions?',
      ...
      '- Could a junior developer maintain this 6 months from now?',
      'Adversarial findings use category "design" with severity "medium" (never block deploy, but flag for attention).',
      '',
      ...
      'Design issues (adversarial) are informational — they do NOT affect the passed flag.',
        ...
        issues: {
          ...
          items: {
            ...
            properties: {
              ...
              category: { type: 'string', enum: ['correctness', 'security', 'style', 'coverage', 'convergence', 'design'] },
              message: { type: 'string' },
